...
let mal = new Mal({ username: settings.malID, password: settings.malPass });
...
MongoClient.connect(settings.mongoUrl, function(err, db) {
    ...
    var commands = {
        ...
        help: function(params, message) {
            message.channel.sendMessage(settings.helpText);
        },
...
` + choices[0].name + ` has ` + choices[0].tokens + ` tokens.`
        ...
        idontspeakweeb: function(params, message) {
            ...
            translate(nihongo, { from: 'ja', to: 'en' }).then(res => {
                message.channel.sendMessage(res.text);
            }).catch(err => {
                console.error(err);
                message.channel.sendMessage('なに？！');
            });
        }
    }
